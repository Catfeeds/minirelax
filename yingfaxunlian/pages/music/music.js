...
Page({
  ...
  xuanze: function (e) {
    ...
    var that = this;
    wx.showActionSheet({
      itemList: ['请选择', '我要提问', '发布笔记'],
      success: function (res) {
        if (res.tapIndex == 2) {
          var pid = that.data.info.id;
          var title = that.data.info.name;
          wx.navigateTo({
            url: '../heart/heart?htype=vclass&title=' + title + '&playId=' + pid,
          })
  ...
  title_Img: function () {
    this.setData({
      showModalStatuss: !this.data.showModalStatuss
    })
  },
  radio: function () {
    var that = this;
    ...
    wx.request({
      ...
      success: function (res) {
        ...
        if (status == 1) {
          ...
          WxParse.wxParse('intro', 'html', intro, that, 3);
          console.log(info.thetime)
          that.setData({
            info: info,
            isFree: info.is_free,
  ...
  ping: function () {
    ...
    wx.navigateTo({
      url: '../heart/heart?playId=' + playId + '&title=' + title,
    });
  },
  ...
  play: function (res) {
    ...
    wx.playBackgroundAudio({
      dataUrl: info.papers,
      title: info.name,
      coverImgUrl: 'http://gzleren.com/miniread/Data/UploadFiles/img.png',
    ...
    function update() {
      wx.getBackgroundAudioPlayerState({
        success: function (res) {
          ...
          if (sumtime != '00:00' && mytime == sumtime) {
            clearInterval(that.data.updateInterval)
              that.setData({
                playing: false,
                formatedPlayTime: '00:00',
                playTime: 0,
