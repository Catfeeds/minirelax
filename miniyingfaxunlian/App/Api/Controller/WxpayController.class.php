...
class WxpayController extends Controller{
	...
	public function wxpay(){
		...
		if ($otype==3) {
			...
			$data['order_type'] = 8;
			$oprice = floatval(1800);
		} else{
			$data['order_type'] = 1;
			$oprice = floatval(599);
		}
		$desc = "原版阅读VIP开通_".$data['order_sn'];
		...
		$input->SetNotify_url('https://gzleren.com/miniread/index.php/Api/Wxpay/notify');
	...
	public function orderhandle($data){
		...
		if ($res) {
			...
			if (intval($check_info['order_type'])==1) {
				$udata['type'] = 2;
			}elseif (intval($check_info['order_type'])==2) {
				$udata['type'] = 3;
			}else {
				$udata['type'] = intval($check_info['order_type']);
			}

			$userInfo = M('user')->where('id='.intval($check_info['uid']))->find();
			if (intval($userInfo['offtime'])>time()) {
				if ($udata['type']==4 || $udata['type']==7) {
					$udata['offtime'] = strtotime('+1 months',intval($userInfo['offtime']));
				} elseif ($udata['type']==5) {
					$udata['offtime'] = strtotime('+3 months',intval($userInfo['offtime']));
				} elseif ($udata['type']==6 || $udata['type']==8) {
					$udata['offtime'] = strtotime('+6 months',intval($userInfo['offtime']));
				}
			}else{
				if ($udata['type']==4 || $udata['type']==7) {
					$udata['offtime'] = strtotime('+1 months',time());
				} elseif ($udata['type']==5) {
					$udata['offtime'] = strtotime('+3 months',time());
				} elseif ($udata['type']==6 || $udata['type']==8) {
					$udata['offtime'] = strtotime('+6 months',time());
				}
			}
			$upuser = M('user')->where('id='.intval($check_info['uid']))->save($udata);
			if (!$upuser) {
				return '用户状态更新失败...';
			}
