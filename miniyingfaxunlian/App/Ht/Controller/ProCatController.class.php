...
class ProCatController extends PublicController{
	...
	*/
	public function _initialize(){
		...
		// 获取所有一级分类，进行关系划分
		$list = $this->pro_cat->where('del=0 AND tid=0')->order('sort asc')->select();
		foreach ($list as $k => $v) {
			$list[$k]['list2'] = $this->pro_cat->where('del=0 AND tid='.intval($v['id']))->order('sort asc')->select();
		}
	...
	*/
	public function add(){
		...
		$list = $this->pro_cat->where('del=0 AND tid=0')->order('sort asc')->field('id,name')->select();
	...
	*/
	public function save(){
		...
		//判断是否已经存在该栏目
		if (!intval($_POST['cid'])) {
			$check_id = $this->pro_cat->where('name="'.trim($_POST['name']).'" AND tid='.intval($tid))->getField('id');
			if ($check_id) {
				$this->error('该分类已存在.');
				exit();
			}
		...
		//上传产品分类缩略图
		if (!empty($_FILES["backimg"]["tmp_name"])) {
			//文件上传
			$info2 = $this->upload_images($_FILES["backimg"],array('jpg','png','jpeg'),"category/".date(Ymd));
		    if(!is_array($info2)) {// 上传错误提示错误信息
		        $this->error($info2);
		        exit();
		    }else{// 上传成功 获取上传文件信息
			    $this->pro_cat->backimg = 'UploadFiles/'.$info2['savepath'].$info2['savename'];
		    }
		}

		if ($tid) {
			$ctype = intval($this->pro_cat->where('id='.intval($tid))->getField('cattype'));
			if ($ctype>0) {
				$this->pro_cat->cattype = $ctype;
			}
		}
	...
	*/
	public function del(){
		...
		if ($res) {
			$url = "Data/".$check_info['img'];
			if (file_exists($url)) {
				@unlink($url);
			}
			$urls = "Data/".$check_info['backimg'];
			if (file_exists($urls)) {
				@unlink($urls);
			}
			$this->redirect('index');
