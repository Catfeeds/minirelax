...
class MoreController extends PublicController{
	...
	public function pweb(){
		if(IS_POST){
			...
			$info = M('web')->where(M('web')->getPk().'='.I('get.id'))->find();
	...
	*/
	public function img_del(){
		...
		$check_info = M('web')->where('id='.intval($id))->find();
		if (!$check_info) {
			echo json_encode(array('status'=>0,'err'=>'图片信息异常！'));
			exit();
		}

		$arr = explode(',', trim($check_info['concent'],','));
		if (in_array($img_url, $arr)) {
			...
			$data = array();
			$data['concent'] = implode(',', $arr);
			$res = M('web')->where('id='.intval($id))->save($data);
			if (!$res) {
